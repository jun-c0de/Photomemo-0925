...
router.get('/posts',
    ...
    async (req, res) => {
        ...
        const items = await Post.find(filter)
            ...
            .skip((+page - 1) * +size)
            .limit(+size)
            .select('title user status fineUrl updatedAt')
...
router.get('/users', authenticateToken, requireRole('admin'), async (req, res) => {
    const { page = 1, size = 20, role, q } = req.body
...
router.patch('/posts/:id', authenticateToken, requireRole('admin'), audit({
    resource: "post",
    action: "update",
...
router.get('/users/:id',authenticateToken,requireRole('admin'),audit({
    resource:'user',
    action:'update',
    getTargetId:(req)=>req.params.id
}),
async(req,res)=>{
    ...
    if(typeof isActive=='Boolean') updates.isAactive = isActive

    if(resetRock){
        updates.failedLginAttempts=0
